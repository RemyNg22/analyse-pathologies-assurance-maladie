...
from Test_LOADER_CSV import charger_echantillon
...
def filtrer_par_sexe(donnees : list[dict], sexe) -> list[dict]:
    return [d for d in donnees if d["Sexe"] == sexe]

def filtrer_par_age(donnees : list[dict], age) -> list[dict]:
    return [d for d in donnees if d["Age"] == age]

def filtrer_par_departement(donnees : list[dict], departement) -> list[dict]:
    return [d for d in donnees if d["Departement"] == departement]

def filtrer_par_annee(donnees : list[dict], annee) -> list[dict]:
...
for c in patho:
    print(f"Pathologie : {c} : {stats_par_tranche_age(donnees, c)}")